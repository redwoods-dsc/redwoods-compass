...
1. Make the change to the JSON file (`src/redwoods_compass_questions*.json`).
    ...
    - **VERSION BUMP NOTE:** If you rename the JSON file to bump the version (e.g., from `_v0.6.json` to `_v1.0.json`), you MUST globally find and replace references to the old filename in `script.js`, `visualization.html`, and schema docs so the application doesn't break. **However**, you should NOT update historical file name references in `docs/json-changelog.md`. Keep old references under their old names, and clearly document the version bump as a bullet point in the current day's log entry.
